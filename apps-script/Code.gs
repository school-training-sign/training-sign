  ...
  const guideSections = [
    {
      ...
      steps: [
        ...
        ['2', '초기 설정과 권한 승인', '새 사본에서 🖊️ 전자서명 관리 → 초기 설정 실행을 누르고, 완료 창의 초기 설정 코드를 보관합니다.', '‘Google에서 확인하지 않은 앱’이 나오면 이 사본을 만든 본인 Google 계정으로 로그인했는지 확인한 뒤 고급 → 학교 연수 전자서명 시스템으로 이동 → 허용을 누릅니다. 본인이 만들지 않은 사본이거나 Google Workspace 정책이 차단하면 중단하고 관리자에게 문의하세요. 사본 소유자만 처음 한 번 승인하며 서명 참여자에게는 이 화면이 나타나지 않습니다.', 134],
    ...
    },
    {
      ...
      steps: [
        ...
        ['2', '기관 설정 입력', '기관 설정에서 학교명·부제목·안내문·개인정보 처리 안내·대표 색상을 저장합니다. 필요하면 QR 안내 캐릭터도 선택합니다.', '개인정보 처리 안내에는 기본 문구가 입력되어 있습니다. 학교의 운영 기준과 보관기간에 맞게 확인·수정하세요. 캐릭터는 공유 화면과 QR 인쇄물에 표시되며 QR 코드 안에는 겹치지 않습니다. 사용 권한이 있는 이미지만 등록하고 얼굴 사진이나 개인정보는 사용하지 마세요.', 96],
