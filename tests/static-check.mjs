...
expect(/Google에서 확인하지 않은 앱/.test(guideSectionOne) && /본인 Google 계정으로 로그인/.test(guideSectionOne) && /고급 → 학교 연수 전자서명 시스템으로 이동 → 허용/.test(guideSectionOne), '미검증 앱에서 사본 소유 계정을 확인하고 승인하는 안내가 누락되었습니다.');
expect(!/개발자 이메일|[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}/i.test(backend + operations), '배포 자료에 개발자 이메일 안내나 개인 계정이 들어가면 안 됩니다.');
expect(/본인이 만들지 않은 사본/.test(guideSectionOne) && /Google Workspace 정책이 차단하면 중단/.test(guideSectionOne) && /사본 소유자만 처음 한 번 승인/.test(guideSectionOne) && /서명 참여자에게는 이 화면이 나타나지 않습니다/.test(guideSectionOne), '다른 사람이 만든 사본·조직 차단 대응 또는 최초 승인 범위 안내가 누락되었습니다.');
